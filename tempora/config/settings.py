...
class Config:
    ...
    suspicious_events: List[str] = field(
        default_factory=lambda: [
            "CreateAccessKey",
            "AttachUserPolicy",
            "AttachGroupPolicy",
    ...
    def load_from_file(cls, path: str) -> "Config":
        ...
        cfg = cls()
        if "min_gap_threshold" in data:
            cfg.min_gap_threshold = int(data["min_gap_threshold"])
        elif "threshold" in data and "min_gap" in data["threshold"]:
            cfg.min_gap_threshold = int(data["threshold"]["min_gap"])

        if "max_reasonable_gap" in data:
            cfg.max_reasonable_gap = int(data["max_reasonable_gap"])
        elif "threshold" in data and "max_gap" in data["threshold"]:
            cfg.max_reasonable_gap = int(data["threshold"]["max_gap"])

        if "timestamp_formats" in data:
            cfg.timestamp_formats = data["timestamp_formats"]
